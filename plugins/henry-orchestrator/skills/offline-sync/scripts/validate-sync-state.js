    ...
    for (const table of tables) {
      try {
        ...
        for (const item of deletedItems) {
          ...
          if (!item.dirty) {
            this.warnings.push(`${table}:${item.id} - deleted but not marked dirty`);
          }

          // Deleted items should have updated_at >= deleted_at
          if (item.updated_at < item.deleted_at) {
            this.errors.push(`${table}:${item.id} - updated_at < deleted_at`);
          }
        }
    ...
    try {
      ...
      for (const log of unsyncedLogs) {
        ...
        if (!['INSERT', 'UPDATE', 'DELETE'].includes(log.operation)) {
          this.errors.push(`sync_log:${log.id} - invalid operation: ${log.operation}`);
        }

        // Check if log has valid timestamp
        if (!log.timestamp || log.timestamp > Date.now()) {
          this.errors.push(`sync_log:${log.id} - invalid timestamp: ${log.timestamp}`);
        }
      }
