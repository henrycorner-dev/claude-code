...
class FlexibleSync {
    ...
    async setupSubscriptions(userId) {
        await this.realm.subscriptions.update(mutableSubs => {
            ...
            mutableSubs.add(
                this.realm.objects('Task').filtered('owner_id == $0', userId),
                { name: 'myTasks' }
            );
            ...
            mutableSubs.add(
                this.realm.objects('UserProfile').filtered('user_id == $0', userId),
                { name: 'myProfile' }
            );
        });
    ...
    async updateSubscriptions(userId, projectIds) {
        await this.realm.subscriptions.update(mutableSubs => {
            ...
            mutableSubs.add(
                this.realm.objects('Task').filtered('project_id IN $0', projectIds),
                { name: 'projectTasks' }
            );
        });
