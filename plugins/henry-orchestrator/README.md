# Henry Orchestrator Plugin

**Version:** 1.0.0

A comprehensive Claude Code plugin that orchestrates complex multi-agent workflows for web, mobile, and game development. Henry Orchestrator coordinates specialized agents, automates quality checks, and streamlines development workflows from initial concept to production deployment.
...
- **Enterprise Features**: SEO, internationalization, PWA conversion, analytics integration
...
cp -r path/to/henry-orchestrator ~/.claude/plugins/
...
- `full-stack-integrator` - End-to-end integration and data flow
...
- `payments-gateway-integrator` - Payment processing integration
- `performance-engineer` - Performance optimization and monitoring
- `data-analytics-engineer` - Analytics implementation and tracking
- `analytics-data-engineer` - Data pipelines and business intelligence
- `product-strategist` - Product strategy and roadmap
- `llm-agent-architect` - AI/LLM integration and agent systems
...
- `state-management` - Redux, Zustand, MobX patterns
...
- `multiplayer-netcode` - Multiplayer networking
...
- `cross-platform-dev` - Cross-platform strategies
...
- `agent-collaboration-synthesis` - Multi-agent coordination
...
See `plugins/henry-orchestrator/hooks/README.md` for detailed hook documentation.
...
/henry-orchestrator:henry-web TaskManagerApp

# The workflow will:
# 1. Analyze context and recommend tech stack
# 2. Scaffold frontend (React/Next/Vue) + backend (Node/NestJS)
# 3. Set up project structure, dependencies, configuration
# 4. Initialize git, setup linting, testing
```

### Example 2: Develop a New Feature End-to-End

```bash
# Orchestrate complete feature development
/henry-orchestrator:henry-feature Add real-time chat with video calling

# Workflow includes:
# - Product strategy (requirements, user stories)
# - UX/UI design (wireframes, mockups)
# - Architecture (frontend, backend, infrastructure)
# - Implementation (code generation, integration)
# - Quality (testing, security review)
# - Performance optimization
# - Deployment planning
```

### Example 3: Mobile App with Push Notifications

```bash
# 1. Scaffold mobile app
/henry-orchestrator:henry-mobile FitnessTracker

# 2. Add push notification support
/henry-orchestrator:henry-push Configure push notifications for fitness reminders
...
### Example 4: Game Development with Multiplayer

```bash
# 1. Create game project
/henry-orchestrator:henry-game BattleRoyaleGame Unity

# 2. Add multiplayer architecture
/henry-orchestrator:henry-multiplayer Implement 100-player battle royale netcode

# 3. Add monetization
/henry-orchestrator:henry-game-monetize Battle pass, cosmetics shop, and ad rewards
```

### Example 5: Optimize Existing Application
...
# Runs:
# - Frontend optimization (bundle size, lazy loading, Core Web Vitals)
# - Backend optimization (database queries, caching, API performance)
# - Mobile optimization (battery, memory, startup time)
# - Infrastructure optimization (CDN, load balancing)
```

### Example 6: Pre-Launch Audit & Deployment
...
/henry-orchestrator:henry-deploy Production deployment
...
The plugin can be configured via `.claude/henry-orchestrator.local.md` in your project:

```markdown
---
default_frontend_framework: next
default_backend_framework: nestjs
enable_hooks: true
hook_strictness: moderate
bundle_size_limit_kb: 250
test_coverage_threshold: 80
---

# Henry Orchestrator Project Configuration

Additional project-specific settings and preferences...
```
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash ${CLAUDE_PLUGIN_ROOT}/hooks/scripts/test-coverage-check.sh",
            "timeout": 60
          }
        ]
      }
...
Configure bundle size thresholds in `hooks/bundle-size-config.json`:

```json
{
  "web": { "warning": 200, "error": 250 },
  "mobile": { "warning": 20, "error": 25 },
  "game": { "warning": 100, "error": 150 }
}
```
...
# Check that command files exist in plugins/henry-orchestrator/commands/
```
...
chmod +x plugins/henry-orchestrator/hooks/scripts/*.sh
...
```bash
# Agents auto-select based on task context
# You can explicitly invoke an agent by name
# Use /henry-team to see recommended agents for your task
```
...
│   ├── backend-engineer.md
│   ├── mobile-app-engineer.md
│   └── ...
├── skills/                       # 33 auto-activating skills
│   ├── frontend-design/
│   ├── backend-development/
│   ├── mobile-ui-design/
│   └── ...
...
This ensures the plugin works regardless of installation location.
...
- **Documentation**: See individual component README files for details
- **Examples**: Check `hooks/`, `skills/`, and `commands/` directories for examples
...
**Claude Code Compatibility:** Latest version
